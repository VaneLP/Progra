...
public class Rectangulo {
    ...
    public void setX1(int x1) {
        if(x1<this.x2  && comprobarLimites(x1)) {
            this.x1 = x1;
        }
        else
    ...
    public void setY1(int y1) {
        if(y1<this.y2 && comprobarLimites(y1)) {
            this.y1 = y1;
        }
        else
            System.err.println("ERROR al instanciar Rectangulo...");
    ...
    public void setX2(int x2) {
        if(x2>this.x1 && comprobarLimites(x2)) {
            this.x2 = x2;
        }
        else
            System.err.println("ERROR al instanciar Rectangulo...");
    ...
    public void setY2(int y2) {
        if(y2>this.y1 && comprobarLimites(y2)) {
            this.y2 = y2;
        }
        else
            System.err.println("ERROR al instanciar Rectangulo...");}
    ...
    public Rectangulo (int x1, int y1, int x2, int y2) {
        ...
        */
        if (x1 < x2 && y1 < y2 && comprobarLimites(x1) && comprobarLimites(y1) && comprobarLimites(x2) && comprobarLimites(y2)) {
            this.x1 = x1;
            this.y1 = y1;
            this.x2 = x2;
            this.y2 = y2;
        ...
        //añadir metodos
        private boolean comprobarLimites(int valor){
    ...
    public static Rectangulo crearRectanguloAleatorio(){
        int azarX1= ThreadLocalRandom.current().nextInt(0,100+1);
        int azarY1= ThreadLocalRandom.current().nextInt(0,100+1);
        int azarX2= ThreadLocalRandom.current().nextInt(azarX1+1,100+1);
        int azarY2= ThreadLocalRandom.current().nextInt(azarY1+1,100+1);
        return new Rectangulo(azarX1,azarX2,azarY1,azarY2);
    }
}
